...
const encryptPhoneNumber = (mobile) => {
...
const decryptPhoneNumber = (encryptedPhoneNumber) => {
  ...
  let mobile = decipher.update(encryptedPhoneNumber, 'hex', 'utf8');
...
module.exports = {
  async beforeCreate(event) {
    ...
    event.params.data.mobile = encryptPhoneNumber(event.params.data.mobile);
  },
  ...
  async afterFindMany(event) {
    ...
    event.result.forEach(item => {
      if (item.mobile) {
        item.mobile = decryptPhoneNumber(item.mobile);
  ...
  async afterFindOne(event) {
    ...
    if (event.result && event.result.mobile) {
      event.result.mobile = decryptPhoneNumber(event.result.mobile);
